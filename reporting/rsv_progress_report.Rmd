...
analytic_dataset_final <-
  ...
  ungroup() %>% 
  mutate(cx = ifelse(lab_confirmed==1, "lab confirmed", "dx only")) %>% 
...
# Section 1: Variables and attrition
...
attrition_2 <- results_tbl(paste0("rsv_sensitivity_", "_attrition")) %>% 
  collect()
...
#        overall = F,
#        data = final_dataset)
#analytic_dataset_final<- analytic_dataset_final%>%mutate(age_cat_at_ce = factor(age_cat_at_ce, levels = c("<6 months", #"6months-1yr", "1-2 yrs", "2-3 yrs", "3-4 yrs", "4-5 yrs")))
label(analytic_dataset_final$age_cat_at_ce)= "Age categories"
table1(~ sex_cat + race_eth_cat + age_cat_at_ce + 
         util_inpatient +
         util_outpatient +
         util_ed + 
