...
This should be updated to the final cohort (including exclusions for having the outcome too soon? right? TODO)
...
final_cohort_demo %>% 
  ...
  summarise(n_pats = n()) %>% 
  ggplot() +
  geom_bar(aes(x=resp_concept, y = n_pats, fill = resp_concept), stat="identity", position="dodge") +
  coord_flip() +
  theme_bw() +
  facet_wrap(~sub_cohort, scales="free", ncol=1) +
  scale_fill_manual(values = wes_palette("FantasticFox1", n=23, type = "continuous")) + 
...
TODO: need to just use the final cohort and report exclusions above in the report
