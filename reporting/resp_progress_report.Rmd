...
tidy.coxme <- function(x, exponentiate = FALSE, conf.int = 0.95, ...) {
  ...
  cohort_entry_end_date = as.Date("2022-10-01")
...
results_tbl(paste0(cohort_1_label, "_cohort_demo")) %>% select(person_id,sub_cohort) %>% left_join(results_tbl(paste0(cohort_1_label, "pmca_summary")) %>% mutate(yrs2 = ifelse(yr_1+yr_2+yr_3 > 0, 1, 0)))  %>% mutate(yrs2=ifelse(is.na(yrs2), 0, yrs2)) %>% group_by(body_system, yrs2, sub_cohort) %>% summarise(n=n()) %>% ungroup() %>% group_by(sub_cohort) %>% mutate(p=n/sum(n)) %>% filter(yrs2==1) %>% ggplot() + geom_bar(aes(x=body_system, y =p, fill=sub_cohort), stat="identity", position="dodge") +coord_flip() +theme_bw() + scale_fill_brewer(palette = "Set2") +
  labs(x="Body system, at least 1 issue in last 3 years", y = "Proportion of cohort")
...
iptw_time <- analytic_dataset_final %>%
  filter(ce_date < as.Date("2023-01-01")) %>% 
    mutate(ce_month = floor_date(ce_date, "month")) %>% 
  mutate(ce_year = floor_date(ce_date, "year")) %>% 
  mutate(ce_month_s = as.character(ce_month)) %>% 
  mutate(ce_year_s = as.character(ce_year)) %>% 
  ...
  do_iptw(cohort_entry_start_date = cohort_entry_start_date,
          comparison_cohort = comparison_group,
          weight_formula = "covid~age_group+sex_cat+race_eth_cat+ce_month_s+pulmonary_respiratory",
          iptw_description = paste0("resp_time_w", comparison_group, "_", outcome))

iptw_no_time <- analytic_dataset_final %>%
    mutate(ce_month = floor_date(ce_date, "month")) %>% 
  mutate(ce_month_s = as.character(ce_month)) %>% 
  filter(resp_outcome_is_reinfection=="0") %>% # TODO allow for re-infection
  mutate(resp30d_or_covid30d = case_when(outcome_resp30d==1 ~ 1,
  ...
  do_iptw(cohort_entry_start_date = cohort_entry_start_date,
          comparison_cohort = comparison_group,
          weight_formula = "covid~age_group+sex_cat+race_eth_cat+pulmonary_respiratory+hospitalized_at_index", ## PULMONARY_RESP?
          iptw_description = paste0("resp_notime_w", comparison_group, "_", outcome))

iptw_time$love_plot

iptw_no_time$love_plot

dataset_time_iptw <- results_tbl( paste0(paste0("resp_time_w", comparison_group, "_", outcome),'_tabc')) %>%
  collect()

dataset_notime_iptw <- results_tbl( paste0(paste0("resp_notime_w", comparison_group, "_", outcome),'_tabc')) %>%
  collect()


models = list()

models$model_notime_unadjusted <- run_logistic_regression(df = dataset_notime_iptw,
                                                       model_formula = paste0(outcome, " ~ covid"),
                                                       weight_method = "none")

models$model_notime_adjusted <- run_logistic_regression(df = dataset_notime_iptw,
                                                         model_formula = paste0(outcome,
                                                                                " ~ covid + race_eth_cat + age_group + sex_cat + hospitalized_at_index + pulmonary_respiratory"),
                                                         weight_method = "none")

models$model_notime_weighted_unadjusted <- run_logistic_regression(df = dataset_notime_iptw,
                                                       model_formula = paste0(outcome, " ~ covid"),
                                                       weight_method = "lr")

# TODO something is wrong with inpatient
models$model_notime_weighted_adjusted <- run_logistic_regression(df = dataset_notime_iptw,
                                                         model_formula = paste0(outcome,
                                                                                " ~ covid + race_eth_cat + age_group + sex_cat + hospitalized_at_index + pulmonary_respiratory"),
                                                         weight_method = "lr")

models$model_time_unadjusted <- run_logistic_regression(df = dataset_time_iptw,
                                                       model_formula = paste0(outcome, " ~ covid"),
                                                       weight_method = "none")

models$model_time_adjusted <- run_logistic_regression(df = dataset_time_iptw,
                                                         model_formula = paste0(outcome,
                                                                                " ~ covid + race_eth_cat + age_group + sex_cat + hospitalized_at_index + pulmonary_respiratory + ce_month_s"),
                                                         weight_method = "none")

models$model_time_weighted_unadjusted <- run_logistic_regression(df = dataset_time_iptw,
                                                       model_formula = paste0(outcome, " ~ covid"),
                                                       weight_method = "lr_s")

# TODO something is wrong with inpatient
models$model_time_weighted_adjusted <- run_logistic_regression(df = dataset_time_iptw,
                                                         model_formula = paste0(outcome,
                                                                                " ~ covid + race_eth_cat + age_group + sex_cat + hospitalized_at_index + pulmonary_respiratory + ce_month_s"),
                                                         weight_method = "lr_s")
...
plot_summs(models$model_notime_unadjusted,
           models$model_notime_adjusted,
           models$model_notime_weighted_unadjusted,
           models$model_notime_weighted_adjusted,
           # models$model_weighted_adjusted_time,
           model.names = c("Unweighted, unadjusted",
                           "Unweighted, adjusted",
                           "Weighted (LR), unadjusted",
...
export_summs(models$model_notime_unadjusted,
           models$model_notime_adjusted,
           models$model_notime_weighted_unadjusted,
           models$model_notime_weighted_adjusted,
           # models$model_weighted_adjusted_time,
...
plot_summs(models$model_time_unadjusted,
           models$model_time_adjusted,
           models$model_time_weighted_unadjusted,
           models$model_time_weighted_adjusted,
           # models$model_weighted_adjusted_time,
           model.names = c("Unweighted, unadjusted",
                           "Unweighted, adjusted",
                           "Weighted (LR), unadjusted",
...
export_summs(models$model_time_unadjusted,
           models$model_time_adjusted,
           models$model_time_weighted_unadjusted,
           models$model_time_weighted_adjusted,
           # models$model_weighted_adjusted_time,
...
models$model_unweighted_adjusted <- run_logistic_regression(df = dataset_with_iptw,
                                                         model_formula = paste0(outcome,
                                                                                " ~ covid + race_eth_cat + age_group + sex_cat + hospitalized_at_index + util_ed + util_outpatient + util_other + pulmonary_respiratory"),
                                                         weight_method = "none")
...
iptw_flu <- analytic_dataset_final %>%
  ...
  do_iptw(cohort_entry_start_date = cohort_entry_start_date,
          comparison_cohort = comparison_group,
          weight_formula = "covid~age_group+sex_cat+race_eth_cat+ce_month_s+pulmonary_respiratory",
          iptw_description = paste0("resp_stdy_m_", comparison_group, "_", outcome))

iptw_flu$love_plot

dataset_with_iptw <- results_tbl( paste0(paste0("resp_stdy_m_", comparison_group, "_", outcome),'_tabc')) %>%
  collect()

modeling_dataset <- dataset_with_iptw %>% 
  mutate(time_to_general30d = case_when(
    outcome_general30d==0 ~ 180,
    TRUE ~ days_from_index_to_first30d_general
  )) %>% 
    mutate(time_to_resp30d = case_when(
    outcome_resp30d==0 ~ 180,
    TRUE ~ days_from_index_to_first30d_resp
  )) 


hazard_unweighted <- survfit(Surv(time_to_general30d, outcome_general30d) ~ covid, data=modeling_dataset)
ggsurvplot(hazard_unweighted)

km_trt_fit <- survfit(Surv(time_to_general30d, outcome_general30d) ~ covid + pulmonary_respiratory, data=modeling_dataset, weights = capped_iptw)
ggsurvplot(km_trt_fit)

resp_unweighted <- survfit(Surv(time_to_resp30d, outcome_resp30d) ~ covid, data=modeling_dataset)
ggsurvplot(resp_unweighted)

resp_weighted <- survfit(Surv(time_to_resp30d, outcome_resp30d) ~ covid + pulmonary_respiratory, data=modeling_dataset, weights = capped_iptw)
ggsurvplot(resp_weighted)

cox_adj <- coxph(Surv(time_to_resp30d, outcome_resp30d) ~ covid + pulmonary_respiratory + hospitalized_at_index + ce_month_s, modeling_dataset, weights=capped_iptw)

cox_unadj <- coxph(Surv(time_to_resp30d, outcome_resp30d) ~ covid + pulmonary_respiratory + hospitalized_at_index, modeling_dataset)

cox_adj_g <- coxph(Surv(time_to_general30d, outcome_general30d) ~ covid + pulmonary_respiratory + hospitalized_at_index + ce_month_s, modeling_dataset, weights=capped_iptw)

cox_unadj_g <- coxph(Surv(time_to_general30d, outcome_general30d) ~ covid, modeling_dataset)

plot_summs(cox_unadj, cox_adj, cox_adj_g, cox_unadj_g, model.names=c(
  "Resp unadjusted",
  "Resp adjusted",
  "General adjusted",
  "General unadjusted"
...
export_summs(cox_unadj, cox_adj, cox_adj_g, cox_unadj_g, model.names=c(
  "Resp unadjusted",
  "Resp adjusted",
  "General adjusted",
  "General unadjusted"
