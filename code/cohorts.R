...
### Make a quick codeset to pick up people with RSV to do the subsequent analysis
flag_rsv_outcome_draft <- function(cohort, outcome_window_start, outcome_window_end) {
  rsv_conditions <-
    load_codeset("dx_noncov_resp_infections") %>% 
    filter(str_detect(concept_name, "Respiratory syncytial virus") |
             str_detect(concept_name, "respiratory syncytial virus")) %>% 
    ...
    left_join(cohort %>% 
                ...
                left_join(cdm_tbl("condition_occurrence") %>% 
                            ...
                            filter(condition_start_date < outcome_window_end, condition_start_date >= outcome_window_start),
  ...
  rsv_tests <- 
    ...
    left_join(cohort %>% select(person_id) %>% 
                left_join(cdm_tbl("measurement_labs") %>% 
                            ...
                            filter(measurement_date < outcome_window_end, measurement_date >= outcome_window_start),
                          by="person_id"), 
