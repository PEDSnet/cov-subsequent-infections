...
#   mutate(min_index_date = as.Date(rsv_evidence_date - days(180))) %>% 
#   mutate(max_index_date = as.Date(rsv_evidence_date - days(30))) %>% 
#   get_flu_evidence_NN(min_date = cohort_entry_start_date, 
#                     max_date = cohort_entry_end_date,
#                     test_only = FALSE,
...
data <- results_tbl("resp_study_analytic_dataset") %>% 
  ...
  union(results_tbl("descriptive_rsv_surge_data") %>% 
          ...
          mutate(disease = "RSV")) %>%  
  exclude_sites(site_excl) %>% collect() %>%
...
ggsave("results/figures_10082024/Figure_1_epi_curve_site_excl.png", width = 10, height=6.5, "units" = "in")  
