...
.run  <- function() {
  ...
  #sample_pats_under_6 <- results_tbl(paste0(cohort_1_label, "_pats_under_6")) %>% 
  # slice_sample(n=3000) %>% 
  # compute_new(indexes=c("person_id"))
  
  # sample_pats_under_6 %>% output_tbl("pat_sample_03_05")
  
  # Check if patients had influenze within 2 months before and 2 months after cohort entry period (dx and lab)
  # Check if patietns EVER had covid  
  # CHeck if patients had other resp within 6 months before and 12 months after cohort entry period
  
  # Covid infection and flu infection was broken down to 3 periods: 
  # prior >= 6 months before index, pre-acute: within 6 months before index, acute: winthin 1 month after index
  # post-acute: within 1-6 months after index, future: more than 6 months after infex
  
  # exclusion: 
  # if index infection is not covid: exclude any covid infections ever
  # if index infection is not flu: exclude flu indection in pre-acute, acute, post-acute
  # if respiratory infection dx within pre-acute, acute, or post-acute period
  
  rslt$triple_cohort <- results_tbl(paste0(cohort_1_label, "_pats_under_6")) %>%
  build_comparison_cohorts_rsv_study_NN(odr_tbl = cdm_tbl("observation_derivation_recover"), # Should probably keep track of the inclusion concept code
  ...
  rslt$other_washout_reasons %>% 
    filter(study_eligible == 1) %>% 
    output_tbl(paste0(cohort_1_label, "_other_washout_reasons"), indexes=c("person_id"), results_tag = FALSE)
  ...
  rslt$final_cohort_demo %>% 
    output_tbl(paste0(cohort_1_label, "_cohort"), indexes=c("person_id"), results_tag = FALSE)
  ...
  pmca_lookup <- produce_pmca_lookup(cohort= results_tbl(paste0(cohort_1_label, "_cohort"), results_tag = FALSE) )
  ...
  pmca_category <- compute_pmca_cats_cons(pmca_summary_tbl = pmca_summary,
                                          cohort_tbl = results_tbl(paste0(cohort_1_label, "_cohort"), results_tag = FALSE)  %>% 
                                            inner_join(cdm_tbl("person") %>% select(person_id, site), by="person_id"))
  
  pmca_category %>% output_tbl(paste0(cohort_1_label, 'pmca_category'), indexes=c('person_id'))
  ...
  pmca_bs_flags <- flag_pmca_bs(cohort_tbl= results_tbl(paste0(cohort_1_label, "_cohort"), results_tag = FALSE),
  ...
  cohort_pmca_flags <- results_tbl(paste0(cohort_1_label, "_cohort"), results_tag = FALSE) %>%
    select('person_id') %>% 
    left_join(pmca_all_flags_tbl %>%
  ...
  output_tbl(pmca_bs_flags,
             paste0(cohort_1_label, 'pmca_bs_flags_full'),
  ...
  results_tbl(paste0(cohort_1_label, "_cohort"), results_tag = FALSE) %>% 
    ...
    generate_analytic_dataset(rsv_outcomes = results_tbl(paste0(cohort_1_label, "_rsv_outcomes")),
                              respiratory_outcomes = results_tbl(paste0(cohort_1_label, "_resp_outcomes")),
  ...
  highrisk_tbl %>% output_tbl(paste0(cohort_1_label, "_highrisk_dx"))
  
  analytic_dataset <- analytic_dataset %>% left_join(results_tbl(paste0(cohort_1_label, "_highrisk_dx")), by = "person_id") %>%
    mutate(highrisk_flag = if_else(is.na(highrisk_flag), FALSE, TRUE))
  analytic_dataset %>% output_tbl(paste0(cohort_1_label, "_analytic_highrisk"))
  
  ## Now get the 4x4 table that Suchitra wants
  analytic_dataset <- results_tbl(paste0(cohort_1_label, "_analytic_highrisk")) 
  analytic_dataset %>% distinct(ce_event, lab_confirmed, sub_cohort)
  
  dx_only <- analytic_dataset %>% filter(sub_cohort == "Influenza")%>%
  ...
  lab_only <- analytic_dataset %>% filter(sub_cohort == "Influenza")%>%
  ...
  dx_lab <- analytic_dataset %>% filter(sub_cohort == "Influenza")%>%
  ...
  analytic_dataset %>% left_join(lab_only, by = "person_id") %>%
    ...
    select(-ce_date.y, -dx_only, -lab_only, -dx_lab) %>%
    output_tbl(paste0(cohort_1_label, "_analytic_final"))
  ...
  output_attrition_table_site_excl(cohort_1_label = "rsv_study", 
                         cohort_entry_start_date = as.Date("2022-03-01"),
                         cohort_entry_end_date = as.Date("2022-07-01"),
                         site_excl)
  results_tbl(paste0(cohort_1_label, "_attrition"))
  message('Done.')
  invisible(rslt)
